...
public class Blinky implements PacmanGhost {
    // make a rectangle var for blinky
    private Rectangle blinky = new Rectangle(15, 15, 15, 15);
    // blinky speed
    int speed = 1;
    // the direction blinky is going in:
    // N = up
    // E = right
    // S = down
    // W = left
    ...
    int east;
    // color of ghost
    Color blinkyColor = new Color(208, 62, 25);
     ...
     * 
     * @return returns the x coordinate of blinky
     */
    public int getX() {
        return this.blinky.x;
    }

    /**
     * get Y coordinate for the ghost
     * 
     * @return returns the y coordinate of blinky
     */
    public int getY() {
        return this.blinky.y;
    }

    /**
     * get rectangle for the ghost
     * 
     * @return returns the rectangle of blinky
     */
     ...
     */
    public void update() {
        // if the direction blinky facing is up or down, change the y value
    ...
    public void drawGhost(Graphics g, BufferedImage image) {
        // set the color
        g.setColor(blinkyColor);
        // draw the image
        g.drawImage(image, this.blinky.x, this.blinky.y, this.blinky.width, this.blinky.height, null);
    }
    ...
    public void ghostLogic(Walls wall, Pacman pacman, Blinky blinky) {
        ...
        if (this.blinky.x % 15 == 0 && this.blinky.y % 15 == 0) {
    ...
    public void pacmanPathFinder(double targetX, double targetY) {
        ...
        double x = this.blinky.x;
        double y = this.blinky.y;
    ...
    public void wallCollision(int[][] board) {
        ...
        int x = this.blinky.x / 15;
        int y = this.blinky.y / 15;
