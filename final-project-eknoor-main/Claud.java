...
public class Claud implements PacmanGhost {
    // make a rectangle var for claud
    private Rectangle claud = new Rectangle(15, 435, 15, 15);
    // claud speed
    int speed = 1;
    // the direction claud is going in:
    // N = up
    // E = right
    // S = down
    // W = left
    ...
    int east;
    // color of ghost
    Color claudColor = new Color(219, 133, 28);
     ...
     * 
     * @return returns the x coordinate of claud
     */
    public int getX() {
        return this.claud.x;
    }
     ...
     * 
     * @return returns the y coordinate of claud
     */
    public int getY() {
        return this.claud.y;
    }
     ...
     * 
     * @return returns the rectangle of claud
     */
     ...
     */
    public void update() {
        // if the direction claud facing is up or down, change the y value
    ...
    public void drawGhost(Graphics g, BufferedImage image) {
        // set the color
        g.setColor(claudColor);
        // draw the image
        g.drawImage(image, this.claud.x, this.claud.y, this.claud.width, this.claud.height, null);
    }
    ...
    public void ghostLogic(Walls wall, Pacman pacman, Blinky blinky) {
        double xDistance = this.claud.x - pacman.getX();
        double yDistance = this.claud.y - pacman.getY();
        double distance = Math.sqrt(Math.pow(xDistance, 2) + Math.pow(yDistance, 2));

        // check everytime ghost is percectly in a 15x15 box on the map
        if (this.claud.x % 15 == 0 && this.claud.y % 15 == 0) {
            ...
            if (!chase) {
                ...
                pacmanPathFinder(0, 500);
            } else {
                // for chasing pacman, chase him until 8 blocks withint its range
                if (distance >= 120) {
                    pacmanPathFinder(pacman.getX(), pacman.getY());
                }
                // then go in random directions
                else {
                    randomPath();
                }
            }
    ...
    public void pacmanPathFinder(double targetX, double targetY) {
        ...
        double x = this.claud.x;
        double y = this.claud.y;
    ...
    public void wallCollision(int[][] board) {
        ...
        int x = this.claud.x / 15;
        int y = this.claud.y / 15;
